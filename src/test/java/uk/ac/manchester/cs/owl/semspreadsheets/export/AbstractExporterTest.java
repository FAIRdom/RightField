...
public class AbstractExporterTest {
	...
	public void getPopulatedValidatedCellDetails() throws Exception {
		...
		List<PopulatedValidatedCellDetails> list = new ArrayList<PopulatedValidatedCellDetails>(
				exporter.getPopulatedValidatedCellDetails());
		...
		assertEquals("http://www.mygrid.org.uk/ontology/JERMOntology#cell_size",
				list.get(0).getTerm().getIRI().toString());
		...
		assertEquals("http://www.mygrid.org.uk/ontology/JERMOntology#dilution_rate",
				list.get(2).getTerm().getIRI().toString());
		assertEquals("http://www.mygrid.org.uk/ontology/JERMOntology#FactorsStudied",
				list.get(2).getEntityIRI().toString());
	...
	public void testPopulatedValidatedCellsForFreeTextProperties() throws Exception {
		...
		for (PopulatedValidatedCellDetails pop : cellDetails) {
			...
			if (!pop.getTextValue().isEmpty()) {
				assertEquals(textValues[x], pop.getTextValue());
				assertEquals(IRI.create("http://www.mygrid.org.uk/ontology/JERMOntology#ECNumber"),
						pop.getOWLPropertyItem().getIRI());
	...
	/**
	 * Uses a spreadsheet created with the latest RightField that does some
	 * formatting of terms (i.e. removing underscores and switching with spaces) and
	 * also contains a cell that contains the same text as an annotated cell, but
	 * isn't marked up with RightField (so that cell should not be returend).
	 * 
	 * @throws Exception
	 */
	@Test
	public void getPopulatedValidatedCellDetails2() throws Exception {
		...
		assertEquals("http://mged.sourceforge.net/ontologies/MGEDOntology.owl#primary_site",
				list.get(0).getTerm().getIRI().toString());
		assertEquals("http://mged.sourceforge.net/ontologies/MGEDOntology.owl#CancerSite",
				list.get(0).getEntityIRI().toString());
		assertEquals(1, list.get(2).getOntologyIRIs().size());
		assertEquals("http://mged.sourceforge.net/ontologies/MGEDOntology.owl",
				list.get(2).getOntologyIRIs().iterator().next().toString());
		assertEquals("http://rest.bioontology.org/bioportal/virtual/download/1131",
				list.get(2).getPhysicalIRIs().iterator().next().toString());
	}

	/**
	 * Tests where 2 ontologies are used, and whether the terms correctly link to
	...
	public void getPopulatedValidatedCellDetails3() throws Exception {
		...
		assertEquals("http://www.mygrid.org.uk/ontology/JERMOntology",
				list.get(0).getOntologyIRIs().iterator().next().toString());
		assertEquals(1, list.get(1).getOntologyIRIs().size());
		assertEquals("http://mged.sourceforge.net/ontologies/MGEDOntology.owl",
				list.get(1).getOntologyIRIs().iterator().next().toString());
		assertEquals("http://rest.bioontology.org/bioportal/virtual/download/1131",
				list.get(1).getPhysicalIRIs().iterator().next().toString());
	}
	...
	public void testGetValidations() throws Exception {
		...
		Collections.sort(list, new Comparator<OntologyTermValidation>() {
			@Override
			public int compare(OntologyTermValidation o1, OntologyTermValidation o2) {
		...
		assertEquals("http://www.mygrid.org.uk/ontology/JERMOntology#Project",
				val.getValidationDescriptor().getEntityIRI().toString());
		...
		assertEquals("http://www.mygrid.org.uk/ontology/JERMOntology#organism",
				val.getValidationDescriptor().getEntityIRI().toString());
		assertEquals("Bacillus subtilis",
				val.getValidationDescriptor().getTerms().iterator().next().getFormattedName());
	}
	...
	/**
	 * A concrete implementation of {@link AbstractExporter} for testing purposes
	 * only.
	 */
