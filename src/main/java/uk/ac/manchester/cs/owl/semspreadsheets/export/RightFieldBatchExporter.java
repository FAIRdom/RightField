...
/**
 * A simple tool for exporting rdf from a batch of spreadsheets in a directory.
 * It is a one off tool, created for a specific purpose, but may have future
 * uses and enhancements.
 * 
 * All Excel files in the provided folder are processed, and rdf generated is
 * put into a file with the same name but .rdf extension. A URI is generated as
 * an identifier based upon the filename.
 * 
 * @author Stuart Owen
 *
 */
public class RightFieldBatchExporter {
				...
				if (file.isFile()) {
					if (file.getName().toLowerCase().endsWith(".xls")) {
						...
						try {
							...
							export = export || JOptionPane.YES_OPTION == JOptionPane.showConfirmDialog(null,
									"The output file " + outFile.getAbsolutePath() + " already exists. Overwrite?");
							if (export) {
	...
	/**
	 * Creates a URI based upon the filename, without the extension - which becomes
	 * rightfield:<filename>
	 * 
	 * @param inFile
