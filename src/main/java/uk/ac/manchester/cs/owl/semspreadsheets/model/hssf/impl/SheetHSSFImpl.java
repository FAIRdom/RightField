...
public class SheetHSSFImpl implements Sheet {
	...
	/**
	 * Creates a Formula List constraint validation, with creates a dropdown list
	 * based on values defined by namedRange(the name of the hidden sheet).
	 */
	...
	private List<HSSFDataValidation> getNoneOntologyTermValidations() {
		...
		for (HSSFDataValidation validation : getValidationData()) {
			...
			for (CellRangeAddress address : validation.getRegions().getCellRangeAddresses()) {
				Validation v = new ValidationImpl(validation.getConstraint().getFormula1(), this,
